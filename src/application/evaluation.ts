...
import type { Capability, CapabilityEvaluation, InterfaceDefinition, InterfaceUse, ProjectionValidationState, RouteEvaluation, SupportState } from "../domain/model";
...
export function validateProjection(capability: Capability, contract: CapabilityContract | undefined, resolved: "RESOLVED" | "UNRESOLVED"): ProjectionValidationState {
  ...
  const hasUnknownConstraint = entityInputs.some((item) => (item.constraints?.length ?? 0) > 0) || capability.invocation.result?.outputs.some((item) => (item.constraints?.length ?? 0) > 0) === true;
  return hasUnknownConstraint ? "UNVALIDATED" : "VALIDATED";
}
...
export function evaluateCapability(capability: Capability, interfaces: readonly InterfaceDefinition[], registry: SemanticRegistry): CapabilityEvaluation {
  ...
  const routes = capability.interfaceUses.map((use) => evaluateRoute(use, interfaces.find((item) => item.id === use.ref), projectionValidation));
...
function evaluateRoute(use: InterfaceUse, definition: InterfaceDefinition | undefined, projection: ProjectionValidationState): RouteEvaluation {
  if (!definition) return { interfaceRef: use.ref, requirement: "SATISFIED", attachment: "SATISFIED", support: "UNSUPPORTED", availability: "UNAVAILABLE", reason: "Interface が存在しません" };
  const requirement = definition.requirements.length > 0 ? "UNKNOWN" : "SATISFIED";
  const attachment = definition.attachment ? "UNKNOWN" : "SATISFIED";
  if (!definition.realization) return { interfaceRef: use.ref, requirement, attachment, support: "UNSUPPORTED", availability: "UNAVAILABLE", reason: "Realization がありません" };
  if (definition.realization.extension.namespace !== "https://relink.dev/ns/arxml/http/0.1" || definition.realization.extension.localName !== "api") return { interfaceRef: use.ref, requirement, attachment, support: "UNKNOWN", availability: "UNKNOWN", reason: "未知の Realization です" };
  try { parseHTTPApi(definition.realization.extension); } catch { return { interfaceRef: use.ref, requirement, attachment, support: "UNSUPPORTED", availability: "UNAVAILABLE", reason: "HTTP Realization が不正です" }; }
  ...
  if (projection === "CONFLICT" || support === "UNSUPPORTED") return { interfaceRef: use.ref, requirement, attachment, support, availability: "UNAVAILABLE", reason: projection === "CONFLICT" ? "Capability projection が CONFLICT です" : "HTTP Mapping が未対応です" };
  if (projection === "UNVALIDATED" || requirement === "UNKNOWN" || attachment === "UNKNOWN" || support === "UNKNOWN") return { interfaceRef: use.ref, requirement, attachment, support, availability: "UNKNOWN", reason: "必要な評価が UNKNOWN です" };
  return { interfaceRef: use.ref, requirement, attachment, support, availability: "READY" };
}
