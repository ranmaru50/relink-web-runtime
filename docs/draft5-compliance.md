...
- Latest result: 5 test files passed, 57 tests passed.
...
| D5-05 | 22–30 | Core XML serialization rules, attributes, required fields, and empty-element restrictions are enforced. | **PARTIAL** | The validator covers the main model, rejects unknown Core attributes/children, and rejects Draft 4 Capability children unless the explicit migration compatibility mode `documentFormat: "draft4"` is selected. That mode is not a Draft 5 conformance mode. The full section-by-section serialization fixture set is not yet present. |
...
| D5-12 | 39–40 | Entity-side Capability projection is checked against Contract inputs, outputs, types, requiredness, format, unit, and Result presence. | **PARTIAL** | Representation comparison, Requirement awareness, and constraint-safe `UNVALIDATED` handling were added. Unimplemented narrowing differences now remain `UNVALIDATED`; full narrowing rules and Contract definition defect reporting are not implemented. |
| D5-13 | 42–49, 68 | Profile identity and required Capability/Property/Identifier/Interface matching are evaluated independently from Runtime availability. | **PARTIAL** | The public `RuntimeDocument.evaluateProfile()` uses the full-document evaluator, evaluates all matching Capability candidates existentially independent of document order, and reports unknown invocation/output evidence as `UNDETERMINED`. Subject scoping, claims, narrowing, and Extension policies remain incomplete. |
...
| D5-17 | 64–67 | Requirements, Support, route evaluation, and availability aggregation use explicit states. | **PARTIAL** | Capability, Contract, and Interface Requirements now compose to `UNKNOWN` without evidence, preventing false `READY`. A pluggable evidence provider and known `UNSATISFIED` path are not implemented. |
| D5-18 | 66–67, 70 | Multiple InterfaceUse routes are evaluated without document-order availability preference; invocation selects an eligible route explicitly. | **PARTIAL** | Stable `routeId` handles, ambiguity rejection, explicit selection, and no automatic retry are implemented and tested. A caller-supplied selection policy beyond the explicit handle is still needed. |
...
| D5-25 | 92–96, Appendices C–E | Security/privacy, namespace evolution, registry guidance, compatibility, error categories, and Draft 4→5 migration are documented and tested. | **PARTIAL** | Same-origin default, fail-closed invocation redirects, no load-time side effect, explicit Draft 4 migration compatibility mode (outside Draft 5 conformance), layered errors, and a required GitHub Actions CI workflow exist. There is no complete security test suite, namespace evolution test matrix, or Appendix C/D/E conformance harness. |
