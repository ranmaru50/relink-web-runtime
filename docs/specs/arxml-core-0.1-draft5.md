...
A present Contract Result MAY list zero Outputs. Additional meaning for such a result shape MUST be supplied through explicitly identified Extension semantics in the Contract's `extension semantics*` information item. Draft 5 defines no separate external-definition reference field or implicit resolution channel for Result. A prose citation alone MUST NOT direct normative resolution or supply a missing machine-readable result definition. Zero listed Outputs MUST NOT by itself make the Contract unusable. This differs from Entity-side XML, where a present `result` MUST contain at least one `output`.

A present Contract Result with no listed Outputs is not automatically equivalent to an absent Result, a no-value Invocation, or permission for arbitrary Entity Outputs. Its normative result meaning and projection rules remain those of the Contract and its explicitly identified Extension semantics. Processors MUST NOT invent Output declarations, insert an empty Entity Result, or guess the shape from prose or AI. Section 40.4 governs comparison when those result semantics are required.
...
For a Contract Result with zero enumerated Outputs, processors MUST use the Contract's normative result and projection rules and any explicitly identified Extension semantics in the Contract model needed to determine the Entity projection. They MUST NOT discover or resolve an additional normative definition from an unmodeled external reference or prose citation. Zero enumeration alone is neither an open-output wildcard nor proof that the Entity should omit Result. Where the necessary comparison semantics are unresolved or unsupported, ProjectionValidation is `UNVALIDATED` unless a separate known conflict takes precedence; processors MUST NOT infer `VALIDATED` or `CONFLICT` solely from the zero count. When those semantics are known, apply their deterministic rules and the applicable named-Output comparisons above. An empty Entity-side XML `result` remains Core-invalid regardless of the Contract's cardinality.
